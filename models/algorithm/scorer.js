...
const get = async (recipePool, userData, selectedRecipeArr) => {
  ...
  const recipeWithFinalScore = recipePoolAfterIngredientBalanceScorer.map(recipe => {
    return {id: recipe.id, finalScore: recipe.shuffleScore + recipe.favoriteScore + recipe.tieredIngredientSaleScore + recipe.ingredientBalanceScore};
  })
